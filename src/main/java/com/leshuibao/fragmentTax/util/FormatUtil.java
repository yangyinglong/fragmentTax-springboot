...
public final class FormatUtil {
    ...
    public static boolean isEmpty(Object obj) {
        ...
        if (obj instanceof Object[]) {
            ...
            for (int i = 0; i < object.length; i++) {
                if (!isEmpty(object[i])) return false;
            }
        }
     ...
     * 使用:toDouble(值,默认值[选填])
     */
    public static double toMaxDouble(String str) {
        double def = 0;
        if (str != null) {
            str = toStringTrim(str);
            int index = str.indexOf("-");
            if (index != -1) {
                def = Double.parseDouble(str.substring(index + 1));
            }
            if (isDecimal(str))
                return Double.parseDouble(str);
        }
     ...
     * 使用:toDouble(值,默认值[选填])
     */
    public static Integer toMaxInt(String str) {
        Integer def = 0;
        if (str != null) {
            str = toStringTrim(str);
            int index = str.indexOf("-");
            if (index != -1) {
                def = Integer.parseInt(str.substring(index + 1));
            }
            if (isNumber(str))
                return Integer.parseInt(str);
        }
     ...
     * xxxxxx
     */
    public static BigDecimal toDecimal(Object str, String MoneyType) {
        DecimalFormat df = new DecimalFormat(MoneyType);
        if (isDecimal(str)) {
            return toDecimal(df.format(toDecimal(str)));
        }

        if (str != null) {
            str = str.toString().replace(" ", "");
            return toDecimal(str.toString());
        }

        return toDecimal(df.format(toDecimal(0)));
    }
    ...
    public static String toStringTrim(Object... args) {
        return toString(args).replaceAll("\\s*", "");
    }
    ...
    public static String toPercent(Double str) {
        return new StringBuffer(Double.toString(str * 100.0000d)).append("%").toString();
    }
    ...
    public static void main(String[] args) {
        System.out.println(toDecimal("2.344444", "##.###"));
    }
}